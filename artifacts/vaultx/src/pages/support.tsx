...
import { MessageCircle, Plus, ChevronRight, Send, Clock, CheckCircle, XCircle } from "lucide-react";
    ...
    <AppLayout title={ticket?.subject ?? "Support"}>
      <div className="flex flex-col h-[calc(100vh-140px)]">
        ...
        <div className="px-4 pt-4 pb-3 border-b border-border">
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5">#{ticket?.id} · {formatDateTime(ticket?.createdAt)}</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {ticket?.messages?.map((msg: any) => (
            <div key={msg.id} className={cn("flex", msg.isAdmin ? "justify-start" : "justify-end")}>
              <div className={cn("max-w-[80%] rounded-2xl px-4 py-2.5 text-sm", msg.isAdmin ? "bg-white border border-border text-foreground rounded-tl-sm" : "bg-primary text-white rounded-tr-sm")}>
                {msg.isAdmin && <p className="text-[10px] font-semibold text-primary mb-1">Support Team</p>}
                <p className="leading-relaxed">{msg.message}</p>
                <p className={cn("text-[10px] mt-1", msg.isAdmin ? "text-muted-foreground" : "text-white/70")}>{formatDateTime(msg.createdAt)}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {ticket?.status !== "closed" ? (
          <div className="px-4 py-3 border-t border-border bg-white">
            <div className="flex gap-2">
              ...
              />
              <Button size="icon" className="h-10 w-10 shrink-0" onClick={() => reply.trim() && sendReply.mutate(reply.trim())} disabled={sendReply.isPending || !reply.trim()}>
                <Send size={15} />
              </Button>
            </div>
            <button onClick={() => closeTicket.mutate()} className="text-[10px] text-muted-foreground mt-2 hover:text-foreground">
              Close ticket
            </button>
          </div>
        ) : (
          <div className="px-4 py-3 border-t border-border bg-slate-50 text-center text-xs text-muted-foreground">
...
export default function SupportPage() {
  ...
  const FAQ = [
    ...
    { q: "How are ROI profits calculated?", a: "Daily profits are calculated based on your invested amount and the daily ROI rate (2.5%–3.0%)." },
    ...
    <AppLayout title="Support">
      <div className="px-4 pt-5 pb-24 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <a href="https://t.me/" target="_blank" rel="noopener noreferrer" className="bg-[#0088cc]/5 border border-[#0088cc]/20 rounded-xl p-3.5 flex items-center gap-3 hover:shadow-sm transition-shadow">
          ...
          <a href="https://wa.me/" target="_blank" rel="noopener noreferrer" className="bg-emerald-50 border border-emerald-200 rounded-xl p-3.5 flex items-center gap-3 hover:shadow-sm transition-shadow">
        ...
        {isLoading ? (
          ...
          <div>
            <h3 className="font-semibold text-sm mb-3">My Tickets</h3>
              ...
              {tickets.map((ticket: any) => {
                ...
                return (
                  <Link key={ticket.id} href={`/support/${ticket.id}`}>
                    <div className="flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50 transition-colors cursor-pointer">
                      <div className={cn("w-8 h-8 rounded-full flex items-center justify-center shrink-0", STATUS_COLORS[ticket.status] ?? "bg-muted")}>
                        <StatusIcon size={14} />
                      </div>
